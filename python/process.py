    ...
    while pcb_list:
        ...
        if time >= pcb_list[0].in_time:
            ...
            if pcb_list[0].need_time == pcb_list[0].cpu_time:
                ...
                pcb_list[0].turn_time = pcb_list[0].out_time - pcb_list[0].in_time
                pcb_list[0].Wturn_time = pcb_list[0].turn_time / pcb_list[0].need_time      
...
def alo(list1):
    ...
    while True:
        ...
        n = input(
            "请选择算法(1、先来先服务  2、短作业优先  3、静态高优先级优先 4、动态高优先级优先) 5、重新录入进程:\n")
        if n == "1":
